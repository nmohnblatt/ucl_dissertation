...
\begin{definition}[BLS Signatures \cite{BLS}]
	A BLS signature scheme $\mathcal{S}_{BLS}$ is composed of three efficient algorithms $\Keygen, \Sign, \Verif$. Let $H_0$ a cryptographic hash function defined as $H_0: \{0,1\}^* \rightarrow \mathbb{G}_0$, and ``$\sample$'' denote the ``choose uniformly at random'' operator, we define the three algorithms as:
	\begin{quote}
		$\Keygen(\kappa):$ Choose $q$, a prime of $\kappa$-bits and three cyclic groups $\Gzero, \Gone, \Gt$ of order $q$ such that there exists a pairing $e : \Gzero \times \Gone \rightarrow \Gt$ (see definition \autoref{def:pairing}). $g_0 \in \Gzero$ and $g_1 \in \Gone$ are generators. Choose uniformly at random $x \sample \mathbb{Z}_q^* $ and set the secret key $\sk \leftarrow x$ and the public key $\pk \leftarrow {g_1}^x$. Output $\sk$ to the message signer and $\pk$ to the receiver. \\
...
\paragraph{Security -- } A left/right constrained PRF is said to be secure if for all probabilistic polynomial time adversaries, the output of the PRF is indistinguishable with a non-negligible advantage from an element chosen uniformly at random in the output domain.
